  ...
  async function doPack(
    ...
    try {
      ...
      if (existingId) {
        const cur = Math.max(
          0,
          parseInt(cards[existingId].packingInfo.quantity || "0", 10) || 0,
        );
        ...
        setColumns((prev) => ({
          ...prev,
          manufactured: prev.manufactured.filter((id) => id !== beforeId),
          stock: prev.stock,
        }));
      } else {
        // 通常の在庫化
  ...
  async function doShip(
    ...
    try {
      ...
      const shipType: ShipType =
        (payload.shipType as ShipType) || "ロジカム出荷";
      // 製造→出荷では location 入力なし
      const loc =
        from === "manufactured"
          ? ""
          : payload.location || item.packingInfo.location || "";
      const maxFromManufactured = item.quantity;
      const maxFromStock = Math.max(
        1,
        Number(item.packingInfo.quantity) || item.quantity,
      );
      const reqQty =
        payload.quantity ||
        (from === "manufactured" ? maxFromManufactured : maxFromStock);
      const qty = Math.max(
        1,
        Math.min(
          reqQty,
